...
const DIMENSION_COLUMN_TO_KEY: Record<string, string> = {
...
function buildDimensionEntries(row: ExperienceImportRow): DimensionEntry[] {
  ...
  for (const [column, key] of Object.entries(DIMENSION_COLUMN_TO_KEY)) {
...
export async function seedExperiencesFromWorkbook(
  ...
  const dimensionRows = await prisma.dimensionIndex.findMany({
    where: {
      index_key: {
        in: Object.values(DIMENSION_COLUMN_TO_KEY),
      },
    },
    select: {
      id: true,
  ...
  const missingDimensionKeys = Array.from(
    new Set(Object.values(DIMENSION_COLUMN_TO_KEY))
